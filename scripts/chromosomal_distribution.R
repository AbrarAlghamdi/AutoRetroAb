...
if (length(args) != 5) {
  stop("Usage: Rscript chromosomal_distribution.R <de_csv> <metadata_tsv> <out_csv> <alpha> <log2fc>")
}
...
if ("chrom" %in% colnames(meta) && !"chromosome" %in% colnames(meta)) {
  meta$chromosome <- meta$chrom
}

required_meta <- c("locus_id", "chromosome")
...
merged <- merge(sig, meta[, c("locus_id", "chromosome")], by = "locus_id")
...
res <- as.data.frame(table(merged$chromosome), stringsAsFactors = FALSE)
colnames(res) <- c("chromosome", "count")
res <- res[order(res$chromosome), ]
